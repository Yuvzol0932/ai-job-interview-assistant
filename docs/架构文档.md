...
| `services/job_aggregator` | 业务层 | 岗位种子数据、本地缓存、RSS/Atom 拉取 | 不调用模型 |
| `services/job_matcher` | 业务层 | 简历与岗位规则匹配、可选 LLM 精排 | 不处理界面逻辑 |
...
```python
JobPosting:
  id: str                        # seed-xxx / rss-xxx
  title / company / category / location: str
  salary / education / experience: str
  requirements / tags: list[str]
  description: str
  source / source_label: str     # seed / rss
  url / posted_at / deadline: str
```

### MatchedJob（岗位匹配结果）

```python
MatchedJob = JobPosting 字段 +
  match_score: int               # 0-100
  match_reasons: list[str]       # 推荐理由
  gap_hints: list[str]           # 简历差距提示
```
...
InterviewReport:
  ...
  overall_impression: str      # 面试官口吻的整体印象
  question_comments: list[dict]  # [{question, comment}] 逐题点评
  growth_advice: list[str]     # 成长建议
  closing: str                 # 收尾鼓励
```
...
主版本界面层改为独立 React 前端，业务层保持不变，新增 FastAPI 网关：

```mermaid
flowchart TB
    WEB[web/ React 前端<br/>Vite + TS + Tailwind<br/>只负责展示与收集输入] --> API[api/ FastAPI 网关<br/>JSON 请求/响应 + 中文错误]
    API --> SVC[services/ 业务层<br/>原有服务全部复用]
    SVC --> LLM[llm/ 模型接口层]
    SVC --> DATA[data/reports 本地报告存储]
```

- `web/` 不直接调用模型或读写文件；所有 AI 操作通过 `api/` 完成。
- 面试流程采用"状态快照往返"：前端保存 `InterviewState` JSON，每个动作调用对应接口，由 Python 状态机更新后返回。
- 报告、简历等错误统一为中文 JSON（`{detail: 提示}`），前端展示为可读提示，不暴露堆栈。
- 企业招聘：`GET /api/jobs` 浏览筛选、`POST /api/jobs/match` 简历匹配、`POST /api/jobs/refresh` 拉取 RSS 源；断网时由 `data/jobs_seed.json` 兜底。
