...
def _render_setup(client) -> None:
    st.caption("选择岗位方向，AI 将像真实面试官一样逐题提问。")
    labels = job_labels()
...
def _render_question(state) -> None:
    st.caption(f"岗位：{state.job_label}")
    st.progress(
        state.answered_count / max(state.total, 1),
        text=f"已答 {state.answered_count} / {state.total} 题",
    )
    ...
    answer = st.text_area(
        "你的回答",
        height=180,
        key=f"answer_{state.current_index}",
